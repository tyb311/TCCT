...
class FTC(nn.Module):
	__name__ = 'gtc'
	def __init__(self, base_cnn, base_vit, out_channels=5, filters=64, flag_gate=True, flag_cnn=True, flag_vit=True, **args):
		super().__init__()
		...
		self.tran_vit0 = nn.Sequential(nn.Conv2d(embed_dims[1], filters, 1,1,0),nn.BatchNorm2d(filters))
		self.tran_vit1 = nn.Sequential(nn.Conv2d(embed_dims[2], filters, 1,1,0),nn.BatchNorm2d(filters))
		self.tran_vit2 = nn.Sequential(nn.Conv2d(embed_dims[3], filters, 1,1,0),nn.BatchNorm2d(filters))
		self.tran_vit3 = nn.Sequential(nn.Conv2d(embed_dims[3], filters, 1,1,0),nn.BatchNorm2d(filters))

		self.tran_cnn0 = nn.Sequential(nn.Conv2d(layer_dims[1], filters, 1,1,0),nn.BatchNorm2d(filters))
		self.tran_cnn1 = nn.Sequential(nn.Conv2d(layer_dims[2], filters, 1,1,0),nn.BatchNorm2d(filters))
		self.tran_cnn2 = nn.Sequential(nn.Conv2d(layer_dims[3], filters, 1,1,0),nn.BatchNorm2d(filters))
		self.tran_cnn3 = nn.Sequential(nn.Conv2d(layer_dims[4], filters, 1,1,0),nn.BatchNorm2d(filters))
		...
		self.dec1 = MPUpBlock(filters, 32)
		self.dec2 = MPUpBlock(filters, 32)
		self.dec3 = MPUpBlock(filters, 32)
		self.dec4 = MPUpBlock(filters, 32)

		self.aux0 = nn.Conv2d(32, out_channels, kernel_size=1)
		self.aux1 = nn.Conv2d(32, out_channels, kernel_size=1)
		self.aux2 = nn.Conv2d(32, out_channels, kernel_size=1)
		self.aux4 = nn.Conv2d(32, out_channels, kernel_size=1)
